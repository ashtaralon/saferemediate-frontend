...
describe("interactive attacker maps", () => {
  ...
  it("wires both Zoom0 attacker lenses to interactive map engines", () => {
    ...
    expect(zoom0).toContain('className="h-full min-h-0 overflow-hidden"')
    ...
    expect(attackPaths).toContain("documentScroll={embedded}")
    expect(attackPaths).toContain('const shellHeight = embedded ? "min-h-[78vh]" : "h-screen"')
    expect(attackPaths).toContain('embedded ? "overflow-visible rounded-xl border border-border"')
    expect(attackPaths).toContain("flex flex-wrap rounded-md border")
    expect(attackPaths).not.toContain("w-max max-w-full")
