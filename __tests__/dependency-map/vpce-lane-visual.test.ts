...
describe("vpce-lane-visual (Bug M)", () => {
  it("counts active vs available in subnet", () => {
    ...
    expect(vpceLaneSubtitle(counts)).toBe("1 active · 3 available in subnet")
  })

  it("handles zero active (KMS-only path, all available)", () => {
    const counts = countVpceLane(["a", "b", "c", "d"], new Set())
    expect(vpceLaneSubtitle(counts)).toBe("0 active · 4 available in subnet")
  })
  ...
  it("styles active vs inactive in architecture view", () => {
    ...
    expect(vpceCardChrome(false, false)).toContain("opacity-60")
    expect(vpceCardChrome(false, false)).toContain("border-dashed")
    expect(vpceCardChrome(false, false)).toContain("ring-amber-400")
  })
