...
describe("Zoom0 Lateral wiring", () => {
  ...
  it("uses the same foothold-first ATLAS model in the full Lateral tab", () => {
    ...
    const map = read("components/attack-paths-v2/atlas-lateral-flow-map.tsx")
    expect(map).toContain("<TrafficFlowMap")
    expect(map).toContain("architectureOverride={architecture}")
