...
export async function GET(request: NextRequest) {
  const backendUrl = `${BACKEND_URL}/api/enforcement/config`

  console.log(`[Enforcement Config Proxy] GET: ${backendUrl}`)
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[Enforcement Config Proxy] Backend error ${response.status}: ${errorText}`)
    ...
    console.error(`[Enforcement Config Proxy] Error:`, error)
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  console.log(`[Enforcement Config Proxy] PUT: ${backendUrl}`)
