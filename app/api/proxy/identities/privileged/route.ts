...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/identities/privileged${params}`, {
      cache: "no-store",
    ...
    if (!res.ok) {
      ...
      console.error(`[proxy] identities/privileged backend returned ${res.status}: ${errorText}`)
    ...
    console.error("[proxy] identities/privileged error:", error.message)
