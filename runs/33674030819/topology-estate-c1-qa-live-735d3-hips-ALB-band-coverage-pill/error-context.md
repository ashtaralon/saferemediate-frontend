...
```
Error: estate map did not mount: Topology risk unavailable
```
...
- generic [active] [ref=e1]:
  ...
  - generic [ref=e35]:
    ...
    - generic [ref=e37]: Topology risk unavailable
    - generic [ref=e38]: HTTP 502
...
> 287 |       throw new Error(`estate map did not mount: ${reason}`)
  ...
  382 |     }
  383 | 
  384 |     // Scroll the Lambda lane when it overflows: the last chip must land inside
  385 |     // a lane body that is at least one chip tall, with both fold pills up.
  386 |     const serverless = inventory.lanes.serverless
  387 |     if (serverless && serverless.scrollHeight > serverless.clientHeight + 4) {
```