...
- main:
  ...
  - text: ☁ AWS Cloud · acct 745783559495 Region · eu-west-1 VPC · vpc-0329e985173bed24f SafeRemediate-Test-… ×3 eu-west-1a eu-west-1b WEB TIER Public · SafeRemediate-Test-Public-1 10.0.1.0/24
  - button "high posture score …Frontend-1": …Frontend-1
  - text: Public · SafeRemediate-Test-Public-2 10.0.2.0/24
  - button "high posture score …Frontend-2": …Frontend-2
  - text: APPLICATION TIER Private · SafeRemediate-Test-Private-App-1 10.0.10.0/24 No workloads Private · SafeRemediate-Test-Private-App-2 10.0.11.0/24
  - button "quiet posture score …App-2": …App-2
  - text: DATABASE TIER Data · SafeRemediate-Test-Private-DB-1 10.0.20.0/24
  ...
  - text: Data · SafeRemediate-Test-Private-DB-2 10.0.21.0/24 No workloads IAM CP IAM · Control plane Roles attached to this VPC derived from instance-profile + USES_ROLE edges · 12 roles · 2 critical gaps · 6 recomputing AlonIAMTest 1/1 unused recomputing · usage edges present, scalar stale AlonIAMTest-traffic-generator USES_ROLE cyntro-demo-ec2-s3-role-least-privilege 6/6 unused recomputing · usage edges present, scalar stale SafeRemediate-Test-App-2 USES_ROLE cyntro-demo-ec2-s3-role-lp-202602081638 6/6 unused recomputing · usage edges present, scalar stale SafeRemediate-Test-App-2 USES_ROLE cyntro-demo-ec2-s3-role-lp-202602081656 6/6 unused recomputing · usage edges present, scalar stale SafeRemediate-Test-App-2 USES_ROLE alon-prod-3tier-app-role 23/27 unused 85% gap · never remediated alon-prod-3tier-web-role 23/27 unused 85% gap · never remediated cyntro-demo-cmk-consumer 5/7 unused 71% gap · never remediated SafeRemediate-Lambda-Remediation-Role 6/10 unused 60% gap · remediation 2026-04-04 PaymentTrafficGenerator SafeRemediate-BehaviorAnalyzer SafeRemediate-ConfidenceScorer + 2 more USES_ROLE · shared cyntro-demo-ec2-s3-role 4/7 unused 57% gap · never remediated SafeRemediate-Test-App-2 via instance profile · USES_ROLE · shared alon-demo-ec2-role 25/46 unused 54% gap · remediation 2026-05-24 CyntroEC2S3Role 0/0 actions recomputing · usage edges present, scalar stale SafeRemediate-Test-Frontend-2 via instance profile · USES_ROLE · shared cyntro-demo-frontend-ssm-role 0/0 actions recomputing · usage edges present, scalar stale SafeRemediate-Test-Frontend-1 via instance profile · USES_ROLE · shared VPC boundary ↑ Internet
  ...
  - button "quiet posture score AlonIAMTest-traffic-generator Lambda": AlonIAMTest-traffic-generator Lambda
  - button "quiet posture score PaymentTrafficGenerator Lambda": PaymentTrafficGenerator Lambda
  - button "quiet posture score …BehaviorAnalyzer Lambda": …BehaviorAnalyzer Lambda
  - button "quiet posture score …ConfidenceScorer Lambda": …ConfidenceScorer Lambda
  - button "quiet posture score …CreateCheckpoint Lambda": …CreateCheckpoint Lambda
  - button "quiet posture score …PrismaWebhook Lambda": …PrismaWebhook Lambda
  - text: Regional · S3 / DDB (18) SafeRemediate-… ×3
  - button "quiet posture score …Executions DynamoDB": …Executions DynamoDB
  - button "quiet posture score …Findings DynamoDB": …Findings DynamoDB
  - button "quiet posture score …Simulations DynamoDB": …Simulations DynamoDB
  - button "quiet posture score alon-demo-data-bucket-745783559495 S3": alon-demo-data-bucket-745783559495 S3
  - button "quiet posture score aws-sam-cli-managed-default-samclisourcebucket-zpixwbu9coth S3": aws-sam-cli-managed-default-samclisourcebucket-zpixwbu9coth S3
  - button "quiet posture score cyntro-demo-analytics-745783559495 S3": cyntro-demo-analytics-745783559495 S3
  - button "quiet posture score cyntro-demo-eu S3": cyntro-demo-eu S3
  - button "quiet posture score cyntro-demo-prod-data-745783559495 S3": cyntro-demo-prod-data-745783559495 S3
  - button "quiet posture score cyntronewtestbucket S3": cyntronewtestbucket S3
  - button "quiet posture score cyntrotest2 S3": cyntrotest2 S3
  - button "quiet posture score impaciq-findings DynamoDB": impaciq-findings DynamoDB
  - button "quiet posture score impaciq-remediation-history DynamoDB": impaciq-remediation-history DynamoDB
  - button "quiet posture score impaciq-scan-status DynamoDB": impaciq-scan-status DynamoDB
  - button "quiet posture score least_privilege_role_state DynamoDB": least_privilege_role_state DynamoDB
  - button "quiet posture score saferemediate-access-logs-745783559495 S3": saferemediate-access-logs-745783559495 S3
  - button "quiet posture score saferemediate-demo-cloudtrail-745783559495 S3": saferemediate-demo-cloudtrail-745783559495 S3
  - button "quiet posture score saferemediate-logs-745783559495 S3": saferemediate-logs-745783559495 S3
  - button "quiet posture score sg_state DynamoDB": sg_state DynamoDB
...
> 166 |       await expect(lane, "the external-destination lane is not drawn by default").toBeVisible()
  ...
  234 |       // The canvas is a horizontal scroll region with a width floor, so at a
  235 |       // narrow viewport the lane can sit to the right of the fold. Bring it
  236 |       // into view before measuring and before the frame: "visible by default"
  237 |       // means no disclosure to open, not that every viewport is wide enough to
  238 |       // hold the whole estate at once.
  239 |       await lane.scrollIntoViewIfNeeded()
  240 |       const laneRect = (await rectOf(lane))!
  241 |       const boundaryRect = await rectOf(page.getByTestId("topology-vpc-boundary-column"))
  242 |       if (boundaryRect) {
  243 |         expect(
  244 |           laneRect.left,
  245 |           `the external lane is not outside the VPC boundary at ${vp.name}·${density}`,
  246 |         ).toBeGreaterThanOrEqual(boundaryRect.right - 1)
  247 |       }
  248 |       // Inside the VIEWPORT once scrolled to, which is the claim that matters:
  249 |       // a lane drawn off the right edge of the window is not on the map.
  250 |       expect(laneRect.left, `lane off the left at ${vp.name}·${density}`).toBeGreaterThanOrEqual(-1)
  251 |       expect(
  252 |         laneRect.right,
  253 |         `lane not brought fully into view at ${vp.name}·${density}`,
  254 |       ).toBeLessThanOrEqual(vp.width + 1)
  255 |       const dataCells = page.locator('[data-tier="data"]')
  256 |       const cells = await dataCells.count()
  257 |       for (let i = 0; i < cells; i++) {
  258 |         const cell = await rectOf(dataCells.nth(i))
  259 |         if (!cell) continue
  260 |         expect(
  261 |           overlap(laneRect, cell),
  262 |           `the external lane overlaps data-tier cell ${i} at ${vp.name}·${density}`,
  263 |         ).toBe(0)
  264 |       }
  265 |       const rail = await rectOf(page.getByTestId("topology-edge-services-rail"))
  266 |       if (rail) {
```