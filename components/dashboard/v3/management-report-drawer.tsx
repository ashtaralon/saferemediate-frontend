...
export function ManagementReportDrawer({
  ...
  const environmentDefaultApplied = useRef(false)
  ...
  useEffect(() => {
    if (!open) {
      environmentDefaultApplied.current = false
      return
    }
    if (environmentDefaultApplied.current || environmentOptions.length !== 1) return
    setSelectedEnvironments([environmentOptions[0]])
    environmentDefaultApplied.current = true
  }, [environmentOptions, open])
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <aside id="management-report-settings" data-mobile-open={settingsPanelOpen ? "true" : "false"} className={`cyntro-no-print absolute inset-y-0 left-0 z-20 w-[min(20rem,calc(100vw-2rem))] shrink-0 overflow-y-auto border-r border-slate-200 bg-white p-5 shadow-2xl xl:static xl:block xl:w-72 xl:shadow-none ${settingsPanelOpen ? "block" : "hidden"}`}>
          ...
          <div className="mt-6 border-t border-slate-200 pt-5">
            ...
            {scopeNarrowed ? <button type="button" onClick={() => { setSelectedEnvironments(environmentOptions.length === 1 ? [environmentOptions[0]] : []); setSelectedCriticalities([]); setSelectedSystems([]); setSystemSearch("") }} className="mt-2 text-[10px] font-semibold text-violet-600">Reset scope</button> : null}
          </div>
