...
function FlowModeToggle({
  ...
  // Architecture is the base canvas (strip + tiers) — not a flow mode.
  // Overlays are evidence only. Attack paths stay available for Blast Radius.
  const opts: { id: EstateFlowMode; label: string; activeColor?: string }[] = [
    { id: "off", label: "None" },
    { id: "all_access", label: "Observed traffic" },
    {
      id: "attack_paths",
    ...
    >
      <div className="flex items-center gap-2 min-w-0 flex-wrap">
        ...
        <span className="text-[9px] shrink-0" style={{ color: "#94A3B8" }}>
          from inventory placement, not traffic
        </span>
      </div>
      <div
...
export function AwsFrame({
  ...
  // Overlays are evidence only (Observed / Attack paths). Architecture is the strip + tiers — no synthetic lines.
  const overlayEdgeList = useMemo(() => {
    if (flowMode === "off") return []
    return overlayEdges ?? trafficEdgesList
  }, [flowMode, overlayEdges, trafficEdgesList])
