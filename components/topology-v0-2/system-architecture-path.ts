/**
 * System architecture path — model for Estate Map (Platform / SRE / IT).
 *
 * Derives the *intended* business-system structure from topology placement
 * (tiers, ingress, regional services). Not an attack path and not observed
 * traffic — those belong to Risk → Attack Paths and Traffic Map.
 *
 * Pure / environment-agnostic: any system with the topology-risk contract.
 */
import type { TopologyNode, TrafficEdge, VpcTopology } from "./types"
...
/**
 * Dimmed structural edges for "System path" flow mode — consecutive hops
 * that have representative node ids. Not observed traffic; not attack paths.
 */
export function architecturePathToTrafficEdges(
  path: SystemArchitecturePath,
  visible: Set<string>,
): TrafficEdge[] {
  const edges: TrafficEdge[] = []
  const hopNodes = path.hops.map(h => {
    if (h.kind === "internet") return ["__igw__"]
    return h.nodeIds.filter(id => visible.has(id) || id === "__igw__").slice(0, 3)
  })

  for (let i = 0; i < hopNodes.length - 1; i++) {
    const srcs = hopNodes[i] ?? []
    const dsts = hopNodes[i + 1] ?? []
    if (srcs.length === 0 || dsts.length === 0) continue
    for (const s of srcs) {
      for (const d of dsts) {
        if (s === d) continue
        if (s !== "__igw__" && !visible.has(s)) continue
        if (d !== "__igw__" && !visible.has(d)) continue
        edges.push({
          source_id: s,
          target_id: d,
          protocol: null,
