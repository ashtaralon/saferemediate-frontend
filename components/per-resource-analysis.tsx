          ...
          {analysisData.data_sources && (
            <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="font-bold text-blue-900">Data Sources</h4>
                {analysisData.data_sources.access_advisor?.available && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-lg p-3 border border-gray-200">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-semibold text-gray-800">CloudTrail</span>
                    <span className="text-xs text-gray-500">
                      ({analysisData.data_sources.cloudtrail?.observation_days || 90} days)
                    </span>
                  </div>
                  <div className="text-sm space-y-1">
                    <div className="flex justify-between">
                      <span className="text-gray-500">Events Analyzed</span>
                      <span className="font-mono font-bold">{analysisData.data_sources.cloudtrail?.events_analyzed?.toLocaleString() || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Permissions Used</span>
                      <span className="font-mono font-bold text-green-600">{analysisData.data_sources.cloudtrail?.used_count || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Permissions Unused</span>
                      <span className="font-mono font-bold text-red-600">{analysisData.data_sources.cloudtrail?.unused_count || 0}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className={`rounded-lg p-3 border ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-semibold text-gray-800">Access Advisor</span>
                    ...
                    {analysisData.data_sources.access_advisor?.available ? (
                      ...
                      <span className="text-xs px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">N/A</span>
                    )}
                  </div>
                  {analysisData.data_sources.access_advisor?.available ? (
                    <div className="text-sm space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono font-bold text-purple-600">
                          {analysisData.data_sources.access_advisor.services_used?.length || 0}
                        </span>
                      </div>
                      {analysisData.data_sources.access_advisor.services_used?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {analysisData.data_sources.access_advisor.services_used.slice(0, 4).map((svc: string, i: number) => (
                            <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-purple-100 text-purple-700">
                              {svc.split(' ').slice(0, 2).join(' ')}
                            </span>
                          ))}
                          {analysisData.data_sources.access_advisor.services_used.length > 4 && (
                            <span className="text-xs text-gray-500">
                              +{analysisData.data_sources.access_advisor.services_used.length - 4} more
                            </span>
                          )}
                        </div>
                      )}
                      {analysisData.data_sources.access_advisor.last_authenticated && (
                        <div className="flex justify-between mt-1">
                          <span className="text-gray-500">Last Active</span>
                          <span className="font-mono text-xs text-green-600">
                            {new Date(analysisData.data_sources.access_advisor.last_authenticated).toLocaleDateString()}
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        ...
                        <span className={`font-bold text-xs px-1.5 py-0.5 rounded ${
                          analysisData.data_sources.access_advisor.confidence === "HIGH"
                            ? "bg-green-100 text-green-700"
                            : "bg-amber-100 text-amber-700"
                        }`}>
                          {analysisData.data_sources.access_advisor.confidence}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500">
                      {analysisData.data_sources.access_advisor?.note || "Access Advisor data not available"}
                    </p>
                  )}
                </div>
              </div>

              {/* High Risk Unused from Access Advisor */}
              {analysisData.data_sources.access_advisor?.available &&
               analysisData.data_sources.access_advisor.high_risk_unused?.length > 0 && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <AlertTriangle className="w-4 h-4 text-red-600" />
                    <span className="font-bold text-red-800 text-sm">
                      {analysisData.data_sources.access_advisor.high_risk_unused.length} High-Risk Unused Permissions
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {analysisData.data_sources.access_advisor.high_risk_unused.slice(0, 6).map((p: any, i: number) => (
                      <span
                        key={i}
                    ...
                    {analysisData.data_sources.access_advisor.high_risk_unused.length > 6 && (
                      <span className="text-xs text-red-600 font-medium">
                        +{analysisData.data_sources.access_advisor.high_risk_unused.length - 6} more
                      </span>
                    )}
                  </div>
                </div>
              ...
              {/* Analysis Note */}
              {analysisData.analysis_note && (
                <p className="mt-3 text-xs text-blue-700 italic">
                  ℹ️ {analysisData.analysis_note}
                </p>
              )}
            </div>
          )}
