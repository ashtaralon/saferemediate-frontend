...
import { Bot, CheckCircle2, RotateCcw, Route, SearchCheck } from "lucide-react"
    ...
    >
      <div className="border-b px-4 py-3" style={{ borderColor: "#D7EEEA", background: "#F0FDFA" }}>
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          >
            ...
            {assisted ? "AI-assisted · engine grounded" : "Engine explanation"}
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-3">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Why this change</div>
            <p className="mt-1 text-xs leading-5 text-slate-700">{explanation.why_this_change}</p>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">What Cyntro found</div>
            <p className="mt-1 text-xs leading-5 text-slate-700">{explanation.current_state}</p>
            <p className="mt-1 text-xs leading-5 text-slate-600">{explanation.scope_summary}</p>
          </div>
        ...
        <div>
          <div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">
            {explanation.readiness === "READY" ? "How Cyntro will run it" : "Proposed sequence after blockers are cleared"}
          </div>
