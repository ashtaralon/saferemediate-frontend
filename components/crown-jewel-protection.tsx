            ...
            {selected && (
              <div className="space-y-6">
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="max-w-3xl">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-[#2D51DA]/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-[#2D51DA]">
                          {selected.resourceType}
                        </span>
                        <span className={`rounded-full border px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] ${severityTone(selected.evidence.lpSeverity)}`}>
                          {selected.evidence.lpSeverity}
                        </span>
                      </div>
                      <h3 className="mt-4 text-2xl font-semibold text-slate-900">{selected.resourceName}</h3>
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      <div className="rounded-2xl border border-white bg-white px-4 py-3 shadow-sm">
                        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">Priority</div>
                        <div className="mt-2 text-2xl font-semibold text-slate-900">{selected.priorityScore}</div>
                      </div>
                      <div className="rounded-2xl border border-white bg-white px-4 py-3 shadow-sm">
                        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">Paths</div>
                        <div className="mt-2 text-2xl font-semibold text-slate-900">{selected.evidence.pathCount}</div>
                      </div>
                      <div className="rounded-2xl border border-white bg-white px-4 py-3 shadow-sm">
                        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">Principals</div>
                        <div className="mt-2 text-2xl font-semibold text-slate-900">{selected.evidence.observedPrincipals.length}</div>
                      </div>
                      <div className="rounded-2xl border border-white bg-white px-4 py-3 shadow-sm">
                        <div className="text-xs uppercase tracking-[0.16em] text-slate-500">LP Gaps</div>
                        <div className="mt-2 text-2xl font-semibold text-slate-900">{selected.evidence.lpGapCount}</div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="mt-5 rounded-[22px] border border-emerald-200 bg-emerald-50 px-4 py-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-semibold text-emerald-900">Recommended first micro-enforcement</div>
                          <div className="mt-1 text-sm text-emerald-800">{selected.recommendedFirstAction.title}</div>
                          <div className="mt-1 text-sm text-emerald-700">{selected.recommendedFirstAction.summary}</div>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                  <h4 className="text-lg font-semibold text-slate-900">Attacker path to this crown jewel</h4>
                  <p className="mt-1 text-sm text-slate-600">
                    This is the clearest behavioral/configuration path we can currently explain from entry to the selected data asset.
                  </p>

                  <div className="mt-5 space-y-5">
                    {selected.paths.map((path) => (
                      <div key={path.id} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
                        <div className="flex items-center justify-between gap-3">
                          <div className="text-sm font-semibold text-slate-900">{path.title}</div>
                          <div className="text-xs uppercase tracking-[0.16em] text-slate-500">
                            {path.observedEvidence.observedLinks} observed links
                          </div>
                        </div>
                        ...
                        <div className="mt-4 overflow-x-auto">
                          <div className="flex min-w-max items-start gap-3">
                            {path.segments.map((segment, index) => (
                              <div key={`${path.id}-${segment.resourceId}-${index}`} className="flex items-start gap-3">
                                <div className="w-[220px] rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm">
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {segment.isInternetExposed && (
                                      <span className="rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-amber-700">
                                        Entry
                                      </span>
                                    )}
                                  </div>
                                  <div className="mt-3 text-sm font-semibold text-slate-900">{segment.resourceName}</div>
                                  {segment.edgeLabel && (
                                    <div className="mt-2 text-xs leading-5 text-slate-500">{segment.edgeLabel}</div>
                                  )}
                                </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-500">
                          {path.observedEvidence.protocols.length > 0 && (
                            <span className="rounded-full bg-white px-3 py-1">Protocols: {path.observedEvidence.protocols.join(", ")}</span>
                          )}
                          {path.observedEvidence.signals.length > 0 && (
                            <span className="rounded-full bg-white px-3 py-1">Signals: {path.observedEvidence.signals.join(", ")}</span>
                          )}
                        </div>
                      </div>
                    ))}
