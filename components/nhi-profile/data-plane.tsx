...
export function DataPlane({ identityName, detail, identity, onRemediate }: DataPlaneProps) {
  ...
  return (
    <>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {store.allowedOperations?.length > 0 ? store.allowedOperations.map((op: string) => (
                              <span key={op} className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ background: `${OP_COLORS[op] || '#6b7280'}15`, color: OP_COLORS[op] || '#6b7280' }}>{op}</span>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {store.observedOperations?.length > 0 ? store.observedOperations.map((op: string) => (
                              <span key={op} className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ background: `${OP_COLORS[op] || '#6b7280'}15`, color: OP_COLORS[op] || '#6b7280', border: `1px solid ${OP_COLORS[op] || '#6b7280'}40` }}>{op}</span>
                      ...
                      {store.unusedOperations?.length > 0 && (
                        <div className="px-4 pb-3">
                          ...
                          {store.recommendation && (
                            <div className="text-xs p-2 rounded" style={{ background: "#f59e0b08", color: "#f59e0b" }}>{store.recommendation}</div>
                          )}
                        </div>
                      )}
                    </div>
      ...
      {/* Existing S3 Policy Analysis Modal */}
      {showS3Modal && selectedBucket && (
