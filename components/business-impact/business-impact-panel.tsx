      ...
      {expanded ? (
        <div className="grid gap-5 border-t border-slate-200 p-4 lg:grid-cols-3">
          ...
          <div>
            ...
            {scenario.regulatory_exposure.length ? <div className="mt-2 space-y-2">{scenario.regulatory_exposure.map((item, index) => <div key={`${item.regime}-${index}`} className="rounded-md border border-slate-200 p-2"><div className="flex items-start justify-between gap-2"><div className="text-xs font-semibold text-slate-800">{item.regime} · {item.exposure_type}</div><span className={`rounded px-1.5 py-0.5 text-[8px] font-bold ${item.applicability === "CONFIRMED" ? "bg-emerald-50 text-emerald-700" : item.applicability === "NEEDS_CONFIRMATION" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}>{item.applicability.replace(/_/g, " ")}</span></div><div className="mt-1 text-[11px] font-semibold text-slate-700">{item.amount != null && item.currency ? formatImpactMoney(item.amount, item.currency) : item.low != null && item.high != null && item.currency ? `${formatImpactMoney(item.low, item.currency)}–${formatImpactMoney(item.high, item.currency)}` : item.formula}</div>{item.calculation_status === "INPUT_REQUIRED" && item.missing_inputs.length ? <div className="mt-1 text-[9px] text-amber-700">Required: {item.missing_inputs.map((value) => value.replace(/_/g, " ")).join(", ")}</div> : null}<p className="mt-1 text-[9px] leading-4 text-slate-400">{item.conditions.join(" ")}</p><div className="mt-1 text-[9px] text-slate-400">{item.rule_version} · source checked {item.source_checked_at}</div><a href={item.source_url} target="_blank" rel="noreferrer" className="text-[9px] font-semibold text-violet-700 underline">Primary source</a></div>)}</div> : <p className="mt-2 text-xs text-slate-500">No potentially applicable rule pack is selected for this scenario.</p>}
          </div>
          <div>
