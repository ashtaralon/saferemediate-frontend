...
import { hardCoverageBlockCode } from "./resolvers/executionReadiness"
...
function decisionChipLabel(
  ...
  if (hardCoverageBlockCode(gap)) return IAM_LP_COPY.chipBlocked
  if (split.conservationError) return IAM_LP_COPY.chipBlocked
  const gate = execution.gate?.decision?.toUpperCase()
  if (gate === "BLOCK" || gate === "EXCLUDE") return IAM_LP_COPY.chipBlocked
  if (gate === "PENDING") return "Verifying safety"
  if (gate === "UNAVAILABLE") return "Safety unavailable"
  if (gate === "MANUAL_REVIEW") return "Manual review required"
  if (
    gate === "REQUIRE_APPROVAL" ||
    gate === "APPROVAL_REQUIRED" ||
    gate === "CANARY_FIRST"
  ) {
    return split.needsApprovalCount > 0
      ? `${split.needsApprovalCount} need approval`
      : "Approval required"
  }
...
export function VerdictHero({ gap, split, execution, llmSummary }: VerdictHeroProps) {
  if (!gap) return null

  const headline =
    gap.summary.unused_count === null
      ? IAM_LP_COPY.authorityUnknown
      : gap.summary.unused_count > 0
        ? IAM_LP_COPY.overPrivileged
        : IAM_LP_COPY.matchesObserved
  const support = IAM_LP_COPY.support(
    gap.summary.total_permissions,
    gap.summary.used_count,
  )
  const undoReady =
  ...
  const decisionBlocked =
    ...
    gate === "PENDING" ||
    gate === "UNAVAILABLE" ||
    split.conservationError ||
    !!hardCoverageBlockCode(gap)
    ...
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950">{headline}</h2>
        ...
        <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-5 py-4">
          ...
          <div className="mt-2 text-sm text-slate-600">
            {gap.summary.used_count ?? "—"} observed · {gap.summary.unused_count ?? "—"} unused
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        >
          ...
          {decisionChipLabel(gap, split, execution)}
        </div>
        <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">
